...
class NotificationsBloc extends Bloc<NotificationsEvent, NotificationsState> {
  NotificationsBloc(this._getNotifications, this._deleteNotification, this._markNotificationRead)
  ...
  Future<void> _onStarted(NotificationsStarted event, Emitter<NotificationsState> emit) async {
    ...
    final result = await _getNotifications(const GetNotificationsParams(page: 1, pageSize: _pageSize));
    result.match(
      (failure) => emit(state.copyWith(status: NotificationsStatus.error)),
      (page) => emit(state.copyWith(status: NotificationsStatus.loaded, items: page.items, hasMore: page.hasMore)),
    );
  }
  ...
  Future<void> _onMoreRequested(NotificationsMoreRequested event, Emitter<NotificationsState> emit) async {
    ...
    final result = await _getNotifications(GetNotificationsParams(page: _page + 1, pageSize: _pageSize));
    result.match(
  ...
  Future<void> _onDeleted(NotificationDeleted event, Emitter<NotificationsState> emit) async {
    ...
    result.match(
      (failure) {},
      (_) => emit(state.copyWith(items: state.items.where((item) => item.id != event.id).toList())),
    );
  }
